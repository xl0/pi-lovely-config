...
const PKG = "@xl0/pi-lovely-config"
...
const cmp = (a: NonNullable<ReturnType<typeof parse>>, b: typeof a) => a[0] - b[0] || a[1] - b[1] || a[2] - b[2]
...
Review those changes before discarding or committing them.
`)
	process.exit(0)
}
...
await $`git commit --only CHANGELOG.md package.json -m ${`chore(release): ${version}`}`
